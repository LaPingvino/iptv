...
def render():
    ...
    for idx, (path, start, dur, label) in enumerate(CLIPS):
        ...
        cmd = [
            ...
            "-i", path,
            "-t", str(dur),
            "-vf", "scale=1024:576:force_original_aspect_ratio=increase,crop=1024:576,setsar=1,fps=25",
    ...
    filter_complex = (
        ...
        # 3.0s Clean Broadcast Logo Slate: ESPERANTO on top, Star in center, BIG TV at the bottom! (1024x576 @ 25fps)
        "color=c=#022c22:s=1024x576:r=25:d=3[title_bg];"
        "[title_bg]drawbox=x=40:y=40:w=944:h=496:color=#10b981:t=4[tb1];"
        "[tb1]drawbox=x=50:y=50:w=924:h=476:color=#064e3b@0.85:t=fill[tb2];"
        "[tb2]drawbox=x=60:y=60:w=904:h=456:color=#fbbf24@0.6:t=2[tb3];"
        "[tb3]drawtext=fontfile=" + font_bold + ":text='★':fontcolor=#fbbf24:fontsize=100:x=(w-text_w)/2:y=100[ts1];"
        "[ts1]drawtext=fontfile=" + font_bold + ":text='E S P E R A N T O':fontcolor=#34d399:fontsize=44:x=(w-text_w)/2:y=220[tt1];"
        "[tt1]drawbox=x=(iw-260)/2:y=280:w=260:h=3:color=#fbbf24:t=fill[td1];"
        "[td1]drawtext=fontfile=" + font_bold + ":text='TV':fontcolor=white:fontsize=96:x=(w-text_w)/2:y=305[title_out];"
